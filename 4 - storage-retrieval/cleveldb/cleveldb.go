...
var notFoundInTableErr = errors.New("key not found in current table")
var notFoundInDBErr = errors.New("key not found")
...
func (db *ClevelDB) Get(key []byte) ([]byte, error) {
	...
	for _, table := range db.tables {
		_, val, _, err := table.getFromSSTable(key)
...
func (db *ClevelDB) RangeScan(start, limit []byte) (Iterator, error) {
	...
	memtableIterator, err := db.mdb.RangeScan(start, limit)
	if err != nil && err != notFoundInTableErr {
		return nil, err
	}
	...
	return &ClevelIterator{
		...
		currentKey: minKeyIterator.Key(),
		currentVal: minKeyIterator.Value(),
		limit:      limit,
	}, nil
}
...
func (i *ClevelIterator) Next() bool {
	...
	if len(i.iterators) == 0 || bytes.Compare(i.currentKey, i.limit) >= 0 {
		return false
	}
	...
	i.currentKey = minIterator.Key()
	i.currentVal = minIterator.Value()
