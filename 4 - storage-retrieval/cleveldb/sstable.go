...
func (table *SSTable) Put(key, value []byte) error {
	//TODO implement me
	panic("implement me")
}

func (table *SSTable) Delete(key []byte) error {
	//TODO implement me
	panic("implement me")
}
...
func (db *ClevelDB) flushMemtable(file *os.File) (*SSTable, error) {
	...
	for current != nil {
		bytesWritten, err := writeKeyValPairToFile(file, current.key, current.val, false)
		if err != nil {
			return nil, err
		}
...
// search : Searches sstable for a given key
...
func (table *SSTable) getFromSSTable(searchKey []byte) ([]byte, []byte, int64, error) {
	file := table.file

	// Find the index block which encompasses the range where the key can be found
	targetBlock := table.index.search(searchKey)
	...
	for currentOffset < endOfBlockOffset {
		...
		} else if string(searchKey) > string(key) {
			// Since keys are sorted, reaching here means the key isn't found in the block
...
func (table *SSTable) RangeScan(start, limit []byte) (Iterator, error) {
	key, val, currentOffset, err := table.getFromSSTable(start)
