...
--    create index employee_manager_salary on employee (manager_id, salary);
    ...
    insert into bonus (emp_id, amount, time) (select 1 + floor(random() * num_employees),
...
/* What is the total bonus amount (and name) for each employee? */
SET enable_hashjoin=off;
explain
select employee.name, sum(amount)
from bonus
         join employee ON bonus.emp_id = employee.emp_id
